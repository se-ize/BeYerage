...
public class BeverageServiceImpl implements BeverageService {
    ...
    private final BeverageRepository beverageJpaRepository;
    ...
    public List<Beverage> findBeveragesWithLoc() {
        return beverageJpaRepository.findAllWithLoc();
    }

    //PK값으로 음료 상세정보 조회
    @Override
    public Beverage findOneById(Long id) {
        Optional<Beverage> findBeverage = beverageJpaRepository.findById(id);
    ...
    public Optional<Beverage> findOneByNameWithLoc(String name) {
        return beverageJpaRepository.findByNameWithLoc(name);
    }
    ...
    public void updateBeverage(Long id, String name, int price, BottleType type, int size, int row, int column) {
        beverageJpaRepository.findByIdWithLoc(id).ifPresent(
    ...
    public void addFrequency(String name) {
        beverageJpaRepository.findByNameWithLoc(name).ifPresent(
