...
touch1 = board1.get_pin('d:22:i')
touch2 = board1.get_pin('d:23:i')
touch3 = board1.get_pin('d:24:i')
touch4 = board1.get_pin('d:25:i')
touch5 = board1.get_pin('d:26:i')
touch6 = board1.get_pin('d:27:i')
touch7 = board1.get_pin('d:28:i')
touch8 = board1.get_pin('d:29:i')
touch8 = board1.get_pin('d:30:i')
...
class BeYerage:
    ...
    def start(self):
        ...
        start = time.time()
        b_number = 0
        TTS_v2.speak(' 지금 부터 터치센서 음성 안내를 시작합니다')
        while True:
            if time.time()- start > 60:
                # 시현용 시간이기에 600 정도가 적당함.
                # print("time :", time.time() - start)
                
                TTS_v1.StartSpeak('서버로 부터 데이터를 갱신하였습니다.')
            ...
            if touch1.read(): #1번
                ...
                speakstory = MakeASentence.connect(b_name)
                print(speakstory)
                # 한글을 영어로 표현하는 걸 자동으로 변환
                TTS_v2.speak(speakstory)
                time.sleep(1)
                continue
...
while True:
    # TTS_v1.StartSpeak('터치 센서 감지를 시작합니다.')
    print('시작합니다')
